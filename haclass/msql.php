...
******************/
    class Mysql{
        ...
        function __construct($host, $user, $pass, $db){
            self::$connection_info = array('host' => 'localhost', 'user' => 'hasystem', 'pass' => 'tyjFRor05mk1fGvZ', 'db' => 'hasystem');
        }
        ...
        public function pagging($table, $where2, $limit){
            ...
            $sql = sprintf("SELECT * FROM ".$table." WHERE ".$where2." ORDER BY status DESC LIMIT ".$limit);
            self::set('last_query', $sql);
            $result = $conn->query($sql);
            $data = array();
            if ($result->num_rows > 0) {
                // output data of each row
                $data['row'] = $result->num_rows;
