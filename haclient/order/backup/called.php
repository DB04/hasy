                    ...
                    <div class="element-box">
                      <div class="table-responsive">
                        <table class="table table-lightborder">
                          ...
                          <tbody>
<?PHP 
$page = addslashes($_GET['page_call']) * 5;
  if($page > 0){
      $page = $page;
  }
$data = $mysql->pagging('donhang','status = 1', $page.', 5');
$num_rows = $data['row'];
...
?>
                            ...
                            <tr>
                              ...
                              <td class="text-right">
                                <?php echo get_name_sale($data[$i]['sale']); ?>
                              </td>
                              <td class="text-right">
                                <?php echo $data[$i]['price'] * $data[$i]['quantity']; ?>
                              </td>
                              <td class="text-right"><a href="http://127.0.0.1/haaction/seller_act.php?chot=<?php echo $data[$i]['id']; ?>&redirect=<?php echo base64_encode(link); ?>">
                                <button class="btn btn-success" type="submit"> CHỐT</button> </a><a href="http://127.0.0.1/haaction/seller_act.php?huy=<?php echo $data[$i]['id']; ?>&redirect=<?php echo base64_encode(link); ?>">
                                <button class="btn btn-warning" type="submit"> THẤT BẠI</button></a>
                              </td>
                     ...
                     <div class="controls-below-table">
                        <div class="table-records-info">
                         Tổng: 
<?php 
$order = $mysql->where('status', 1)->get('donhang');
$all = $mysql->num_rows(); 
$trang = floor($all/5+1);
echo $all.' đơn';
?>                        </div>
...
<?php for($i=1;$i<$trang+1;$i++){ ?>
                            ...
                            <li>
                              <a href="<?php echo link;?>&page_call=<?php echo $i-1; ?>"><?php echo $i ?></a>
                            </li>
<?php } ?>
